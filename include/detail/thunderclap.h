...
{
template <typename T>
thunderclap<T> *thunderclap<T>::shared_instance()
{
...
template <typename T>
bool thunderclap<T>::available(std::string name)
{
	bool result = false;
...
template <typename T>
void thunderclap<T>::clap(std::string name, T x)
{
...
template <typename T>
std::string thunderclap<T>::watch(std::string name, std::function<void(T x)> f)
{
	std::string hash;
...
template <typename T>
void thunderclap<T>::dispose(std::string name, std::string hash)
{
	if (this->available(name)) {
		auto m = this->table[name];
		auto it = m->find(hash);
...
template <typename T>
void thunderclap<T>::dispose_all(std::string name)
{
	if (this->available(name)) {
		auto m = this->table[name];
		m->clear();
